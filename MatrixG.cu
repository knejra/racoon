...
// conv_im2col
template <typename Dtype>
__global__ void im2colKern(const int n, const Dtype *pic, const int height, const int width, const int ksize, const int pad,
                           const int stride, const int colHeight, const int colWidth, Dtype* colPic) 
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
